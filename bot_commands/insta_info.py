...
async def insta_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Safely handle biography text
        bio_text = str(formatted_data['biography']) if formatted_data['biography'] != 'N/A' else 'No bio available'
        bio_display = bio_text[:100] + '...' if len(bio_text) > 100 else bio_text
...
━━━━━━━━━━━━━━━━━━━━━━━━━"""
        ...
        error_msg = '🚨 Network error occurred. Please try again.'
        if 'loading_msg' in locals():
...
async def handle_reset_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            ...
            print(f"Error parsing email from response: {email_error}")
        
        # Format response in the requested style
        if response.status_code == 200:
            result_text = f"""•𝗣𝗮𝘀𝘀𝘄𝗼𝗿𝗱 𝗥𝗲𝘀𝗲𝘁 𝗟𝗶𝗻𝗸 𝗦𝗲𝗻𝘁 𝗦𝘂𝗰𝗰𝗲𝘀𝘀𝗳𝘂𝗹𝗹𝘆!

≭ 𝗘𝗺𝗮𝗶𝗹 𝗔𝘀𝘀𝗼𝗰𝗶𝗮𝘁𝗲𝗱: `{associated_email}`
≭ 𝗥𝗲𝗾𝘂𝗲𝘀𝘁𝗲𝗱 𝗕𝘆: `{user_name}`
≭ 𝗕𝗼𝘁 𝗕𝘆 / 𝗗𝗲𝘃: @meta\\_server
≭ 𝗧𝗶𝗺𝗲 𝗧𝗮𝗸𝗲𝗻: `{time_taken}` 𝘀𝗲𝗰𝗼𝗻𝗱𝘀"""
        else:
            result_text = f"""•𝗣𝗮𝘀𝘀𝘄𝗼𝗿𝗱 𝗥𝗲𝘀𝗲𝘁 𝗙𝗮𝗶𝗹𝗲𝗱!
...
≭ 𝗘𝗿𝗿𝗼𝗿: `{response.status_code}`"""
        ...
        error_text = f"""•𝗣𝗮𝘀𝘀𝘄𝗼𝗿𝗱 𝗥𝗲𝘀𝗲𝘁 𝗙𝗮𝗶𝗹𝗲𝗱!
...
def setup_insta_commands(app, admin_id=None, channel_id=None):
